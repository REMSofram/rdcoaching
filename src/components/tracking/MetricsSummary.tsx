...
import IndicatorCard from '@/components/mobile/IndicatorCard';
...
export function MetricsSummary({ className = '', clientId }: MetricsSummaryProps) {
    ...
    return (
      <>
        ...
        <div className={`md:hidden space-y-3 ${className}`}>
          {[...Array(5)].map((_, i) => (
            <div key={i} className="bg-gray-50 p-4 rounded-lg border border-gray-100 animate-pulse">
        ...
        {/* Desktop skeleton */}
        <div className={`hidden md:grid grid-cols-5 gap-3 ${className}`}>
          {[...Array(5)].map((_, i) => (
  ...
  return (
    <>
      ...
      <div className={`md:hidden space-y-3 ${className}`}>
        <IndicatorCard title="Dernier poids" value={metrics.lastWeight > 0 ? `${metrics.lastWeight} kg` : '-'} color="blue" />
        <IndicatorCard title="Énergie (7j)" value={metrics.avgEnergy > 0 ? metrics.avgEnergy.toFixed(1) : '-'} color="green" />
        <IndicatorCard title="Sommeil (7j)" value={
          <span>
            {metrics.avgSleep > 0 ? `${metrics.avgSleep.toFixed(1)}h` : '-'}
            {metrics.avgSleepQuality > 0 && <span className="text-sm text-purple-700 ml-1">• {metrics.avgSleepQuality.toFixed(1)}/5</span>}
          </span>
        } color="purple" />
        <IndicatorCard title="Séances (7j)" value={metrics.weeklySessions > 0 ? metrics.weeklySessions : '-'} color="amber" />
        <IndicatorCard title="Série actuelle" value={metrics.currentStreak > 0 ? `${metrics.currentStreak}j` : '-'} color="red" />
      </div>

      {/* Desktop grid indicators */}
      <div className={`hidden md:grid grid-cols-5 gap-3 ${className}`}>
