          ...
          <div className="flex-1">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <span className="px-3 py-1 bg-blue-100 text-blue-800 text-sm font-medium rounded-full">
                {profile?.goal || 'Objectif non défini'}
              </span>
              {profile?.starting_date && (
                <span className="px-3 py-1 bg-green-100 text-green-800 text-sm font-medium rounded-full">
                  Membre depuis {new Date(profile.starting_date).toLocaleDateString('fr-FR')}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="text-gray-700">
              {profile?.about || 'Ajoutez une description plus détaillée sur vous et vos objectifs.'}
            </p>
          </div>
        </div>
      </div>
